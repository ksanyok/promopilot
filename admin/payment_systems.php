        ...
        switch ($gatewayCode) {
            case 'monobank':
                $config = [
                    ...
                    'invoice_lifetime' => (int)($_POST['monobank_invoice_lifetime'] ?? 900),
            ...
            case 'binance':
                $config = [
                    ...
                    'environment' => in_array(strtolower((string)($_POST['binance_environment'] ?? 'production')), ['sandbox', 'test']) ? 'sandbox' : 'production',
                    'terminal_type' => strtoupper(trim((string)($_POST['binance_terminal_type'] ?? 'WEB'))),
                ];
                break;
            default:
...
<div class="card shadow-sm mb-4">
    <div class="card-body">
        <h1 class="h3 mb-0"><?php echo __('Платёжные системы'); ?></h1>
        <p class="text-muted mb-0"><?php echo __('Управляйте подключениями к платёжным шлюзам и отслеживайте транзакции клиентов.'); ?></p>
    </div>
</div>
            ...
            <form method="post">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" id="monobank-token" name="monobank_token" value="<?php echo htmlspecialchars($monobank['config']['token'] ?? ''); ?>" placeholder="live_xxxxxxxxx">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label" for="monobank-merchant"><?php echo __('Merchant ID (опционально)'); ?></label>
                        <input type="text" class="form-control" id="monobank-merchant" name="monobank_merchant_id" value="<?php echo htmlspecialchars($monobank['config']['merchant_id'] ?? ''); ?>">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label" for="monobank-destination"><?php echo __('Назначение платежа'); ?></label>
                        <input type="text" class="form-control" id="monobank-destination" name="monobank_destination" value="<?php echo htmlspecialchars($monobank['config']['destination'] ?? ''); ?>" placeholder="PromoPilot пополнение">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label" for="monobank-redirect"><?php echo __('Redirect URL после оплаты'); ?></label>
                        <input type="url" class="form-control" id="monobank-redirect" name="monobank_redirect_url" value="<?php echo htmlspecialchars($monobank['config']['redirect_url'] ?? ''); ?>" placeholder="<?php echo htmlspecialchars(pp_url('client/balance.php')); ?>">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label" for="monobank-lifetime"><?php echo __('Время жизни счёта (сек)'); ?></label>
                        <input type="number" class="form-control" id="monobank-lifetime" name="monobank_invoice_lifetime" value="<?php echo (int)($monobank['config']['invoice_lifetime'] ?? 900); ?>" min="60" max="86400">
                    </div>
                    <div class="col-md-8">
                        ...
                        <div class="input-group">
                            ...
                            <button class="btn btn-outline-secondary" type="button" onclick="navigator.clipboard?.writeText('<?php echo htmlspecialchars(pp_payment_gateway_webhook_url('monobank')); ?>');">
            ...
            <form method="post">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" id="binance-api-key" name="binance_api_key" value="<?php echo htmlspecialchars($binance['config']['api_key'] ?? ''); ?>" placeholder="pay_xxxxxxxxx">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label" for="binance-api-secret"><?php echo __('API Secret'); ?></label>
                        <input type="password" class="form-control" id="binance-api-secret" name="binance_api_secret" value="<?php echo htmlspecialchars($binance['config']['api_secret'] ?? ''); ?>" placeholder="••••••">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label" for="binance-merchant"><?php echo __('Merchant ID (опционально)'); ?></label>
                        <input type="text" class="form-control" id="binance-merchant" name="binance_merchant_id" value="<?php echo htmlspecialchars($binance['config']['merchant_id'] ?? ''); ?>">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label" for="binance-return"><?php echo __('Return URL после оплаты'); ?></label>
                        <input type="url" class="form-control" id="binance-return" name="binance_return_url" value="<?php echo htmlspecialchars($binance['config']['return_url'] ?? ''); ?>" placeholder="<?php echo htmlspecialchars(pp_url('client/balance.php')); ?>">
                    </div>
                    <div class="col-md-4">
                        ...
                        <select class="form-select" id="binance-environment" name="binance_environment">
                            <option value="production" <?php echo (empty($binance['config']['environment']) || $binance['config']['environment'] === 'production') ? 'selected' : ''; ?>><?php echo __('Продакшен'); ?></option>
                            <option value="sandbox" <?php echo (!empty($binance['config']['environment']) && $binance['config']['environment'] !== 'production') ? 'selected' : ''; ?>><?php echo __('Песочница'); ?></option>
                        </select>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label" for="binance-terminal"><?php echo __('Тип терминала'); ?></label>
                        <input type="text" class="form-control" id="binance-terminal" name="binance_terminal_type" value="<?php echo htmlspecialchars($binance['config']['terminal_type'] ?? 'WEB'); ?>" placeholder="WEB">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label"><?php echo __('Webhook URL'); ?></label>
                        <div class="input-group">
                            <input type="text" class="form-control" value="<?php echo htmlspecialchars(pp_payment_gateway_webhook_url('binance')); ?>" readonly>
                            <button class="btn btn-outline-secondary" type="button" onclick="navigator.clipboard?.writeText('<?php echo htmlspecialchars(pp_payment_gateway_webhook_url('binance')); ?>');">
                                <i class="bi bi-clipboard"></i>
                            </button>
                        </div>
