...
const { sleep } = require('./lib/puppeteerUtils');

const config = {
  slug: 'rentry',
  baseUrl: 'https://rentry.co/',
  contentFormat: 'markdown',
  // Rentry uses CodeMirror editor; wait for it
  waitForSelector: '.CodeMirror, form textarea',
  titleSelectors: ['input#id_title', 'input[name="title"]'],
  contentSelectors: [
    // Rentry new editor (hidden textarea + CodeMirror)
    'textarea#id_text', 'textarea[name="text"]',
    // Generic fallbacks used in other paste sites
    'textarea#id_content', 'textarea[name="content"]'
  ],
  // Help the generic filler to consider contenteditable in CodeMirror
  editableSelectors: [
  ...
  // Custom content fill: set Markdown in CodeMirror and metadata in #metadata_text
  async fillContent({ page, article, variants, job, logLine }) {
    const meta = job && (job.meta || job.page_meta) ? (job.meta || job.page_meta) : {};
    const rentryMeta = (meta && typeof meta.rentry === 'object') ? meta.rentry : {};

    // 1) Prepare metadata block (same logic as prepareBody but only lines)
    const lines = [];
    try {
      ...
      const descr = clip(meta.description || meta.desc || meta.summary || plain, 160); // strict 160 for rentry
    ...
    // 2) Set metadata textarea (#metadata_text) even if hidden
    try {
      const metadataText = lines.join('\n');
      await page.evaluate((val) => {
        const ta = document.querySelector('#metadata_text, textarea[name="metadata"]');
        if (ta) {
          ta.value = val;
          try { ['input','change','keyup','blur','paste'].forEach(evt => ta.dispatchEvent(new Event(evt, { bubbles: true }))); } catch(_) {}
        }
      }, metadataText);
    } catch (_) {}

    // 3) Set content body in CodeMirror if present, else textarea
    const markdown = variants && variants.markdown ? variants.markdown : '';
    let filled = false;
    try {
      filled = await page.evaluate((val) => {
        const cmWrap = document.querySelector('.CodeMirror');
        const cm = cmWrap && cmWrap.CodeMirror ? cmWrap.CodeMirror : null;
        if (cm && typeof cm.setValue === 'function') {
          cm.setValue(String(val || ''));
          try { cm.refresh && cm.refresh(); } catch(_) {}
          return true;
        }
        const t = document.querySelector('#id_text, textarea[name="text"]');
        if (t) {
          t.value = String(val || '');
          try { ['input','change','keyup','blur','paste'].forEach(evt => t.dispatchEvent(new Event(evt, { bubbles: true }))); } catch(_) {}
          return true;
        }
        return false;
      }, markdown);
    ...
    // 4) Optional: set custom URL slug and edit code from meta
    try {
      const slug = meta.slug || job.slug || '';
      const edit = meta.edit_code || meta.edit || '';
      if (slug || edit) {
        await page.evaluate((s, e) => {
          const urlInp = document.querySelector('#id_url, input[name="url"]');
          const editInp = document.querySelector('#id_edit_code, input[name="edit_code"]');
          if (s && urlInp) { urlInp.value = s; try { ['input','change','keyup','blur'].forEach(evt => urlInp.dispatchEvent(new Event(evt, { bubbles: true }))); } catch(_) {} }
          if (e && editInp) { editInp.value = e; try { ['input','change','keyup','blur'].forEach(evt => editInp.dispatchEvent(new Event(evt, { bubbles: true }))); } catch(_) {} }
        }, slug, edit);
      }
    } catch (_) {}

    if (!filled) {
      logLine && logLine('Rentry fillContent fallback failed', {});
    }
    // tiny pause to let preview/validator catch up
    await sleep(120);
    return !!filled;
  },
  
  // Give more time for result redirect
